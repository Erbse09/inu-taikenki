(() => {
  ...
  box.innerHTML = '<div class="dist-title">📊 この条件の体験分布</div><p class="dist-note">条件に一致した体験が、どの商品に何件あるかを表示します。件数の多さはおすすめ順位・満足度・商品の優劣を意味しません。</p><div class="dist-list" aria-live="polite">読み込み中…</div>';
  status.insertAdjacentElement('afterend', box);

  const style = document.createElement('style');
  style.textContent = '.experience-distribution{margin:0 0 14px;padding:13px;background:#fffdf9;border:1px solid #eee3d8;border-radius:14px}.dist-title{font-size:12px;font-weight:900;color:#51463f}.dist-note{margin:3px 0 9px;font-size:8px;line-height:1.65;color:#958981}.dist-list{display:grid;gap:6px}.dist-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:7px 9px;background:#faf6f1;border-radius:9px}.dist-name{font-size:9px;font-weight:800;overflow-wrap:anywhere}.dist-count{font-size:9px;font-weight:900;color:#d97828;white-space:nowrap}.dist-empty{font-size:9px;color:#958981}.dist-more{font-size:8px;color:#958981;text-align:right}';
  ...
  function render() {
    ...
    if (!ranked.length) { list.innerHTML = '<div class="dist-empty">この条件では商品別の体験分布を表示できません。</div>'; return; }
    const shown = ranked.slice(0,8);
    list.innerHTML = shown.map(([name,count]) => '<div class="dist-row"><span class="dist-name">'+esc(name)+'</span><span class="dist-count">'+count+'件</span></div>').join('') + (ranked.length>8 ? '<div class="dist-more">ほか '+(ranked.length-8)+'商品</div>' : '');
  }
